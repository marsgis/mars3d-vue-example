export function addCityGraphics(graphicLayer) {

  // 模型的中心点
  const position = [117.221764, 31.82394, 0] // 用于围绕旋转 + 中心点扩散 + 旋转的图片
  ...
  const divgraphic = new mars3d.graphic.DivGraphic({
    ...
    style: {
      html: `<div class="marsBlackPanel">
          <div class="marsBlackPanel-text">合肥市政府Mars3D</div>
      </div>`,
  ...
  const scanCircle = new mars3d.graphic.CircleEntity({
    position: Cesium.Cartesian3.fromDegrees(117.221764, 31.82394, 24.2),
    style: {
      radius: 480.0,
  ...
  // 远眺的线 ,数据获取的pointArr
  mars3d.Util.fetchJson({ url: "config/bijiashan.json" })
    .then((result) => {
      const pointArr = []
...
// 智慧社区
export function addGraphics(graphicLayer) {
  // // 创建矢量数据图层
...
function randomPoint() {
  const jd = random(117.215003 * 1000, 117.229294 * 1000) / 1000
  const wd = random(31.817991 * 1000, 31.828383 * 1000) / 1000
