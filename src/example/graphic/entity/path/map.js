...
function addDemoGraphic4(graphicLayer) {
  const position = {
    ...
    speed: 260,
    list: [
      [117.164041, 31.745533, 11.9],
      [117.197197, 31.777618, 25.6],
      [117.205486, 31.737966, 19.3],
      [117.230346, 31.77424, 23.9],
      [117.245753, 31.73741, 23.7]
    ],
    forwardExtrapolationType: Cesium.ExtrapolationType.HOLD // 在最后1个结束时间之后，NONE时不显示，HOLD时显示结束时间对应坐标位置
  }
  const graphic = new mars3d.graphic.PathEntity({
    ...
    style: {
      width: 5,
      color: "#0073FF",
      leadTime: 0 // 预先显示时长（单位：秒），比如不显示前方路线，可以设置为0
    },
    model: {
      url: "https://data.mars3d.cn/gltf/mars/wrj.glb",
      scale: 0.5,
  ...
  const graphic1 = new mars3d.graphic.PathEntity({
    position: position,
    style: {
      width: 3,
      ...
      trailTime: 0 // 保留历史时长（单位：秒），尾部保留的时长对应的坐标
    },
    attr: { remark: "示例4" }
  })
