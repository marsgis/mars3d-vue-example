(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["chunk-6914ed44"],{
...
/* harmony default export */ var listvue_type_script_lang_ts_setup_true = (/*#__PURE__*/Object(vue_runtime_esm_bundler["defineComponent"])({
  setup: function setup(__props) {
    ...
    var checkedChange = function checkedChange(keys, item) {
      var node = item.node;
      var layer = layersObj[node.key];
      var isChildern = node.children;
    ...
    function initTree() {
      var layers = treeModelData; // 遍历出config.json中所有的basempas和layers

      for (var i = layers.length - 1; i >= 0; i--) {
        var layer = mapWork.createLayer(mapWork.createLayer(layers[i])); // 创建图层
    ...
    function findChild(parent, list) {
      return list.filter(function (item) {
        ...
        if (item.pid = parent.id) {
          ...
          var nodeLayer = mapWork.createLayer(item); // 创建图层
...
// CONCATENATED MODULE: ./src/example/layer-tileset/type/list/index.vue?vue&type=script&lang=ts&setup=true
 
// CONCATENATED MODULE: ./src/example/layer-tileset/type/list/index.vue



const __exports__ = listvue_type_script_lang_ts_setup_true;
